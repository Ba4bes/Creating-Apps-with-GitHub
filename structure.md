...
4. Wait for the Codespace to open (typically 60-90 seconds, may queue briefly)
5. In the terminal, run the start command shown in the repo README
6. Click the Ports tab, click the globe icon next to the forwarded port
7. You should see the running app in a browser tab

Expected output: app visible in browser.

If Codespaces fails at any step: clone the repo locally, run the one-command setup in the README, open in VS Code.

### Core -- Self-directed

Get the workshop repo running in a browser tab using Codespaces. Use the `.devcontainer` folder and the README as your references. Local VS Code is acceptable if Codespaces does not work.

### Extra Challenge -- Guided

1. In the Codespace, open `.devcontainer/devcontainer.json`
2. Add one VS Code extension to the `extensions` array (find extension IDs in the VS Code marketplace)
3. Open the command palette, run "Rebuild Container"
4. Verify the extension is installed after rebuild
5. Read the language folders (node/, python/, dotnet/) and identify what is shared between them versus what is language-specific

### Extra Challenge -- Self-directed

Modify the devcontainer to add a tool or extension you actually use. Understand the relationship between the three language folders and the shared frontend before the debrief.
...
## 10:30 - 11:00 | Copilot: tiers, limits, and working within them (30 min)
...
## 11:00 - 12:00 | Lab 1: Custom instructions, prompt files, and skills demo (60 min)
...
## 12:00 - 12:10 | Morning wrap

Show what exists in the repo now: devcontainer, instructions file, prompt files. Preview the afternoon: "This afternoon you implement a feature using these tools, run it through CI, and ship it to a live URL. Pick your language folder if you have not."
...
## 13:10 - 13:30 | Issues as specifications + agentic workflow demo (20 min)
...
## 13:30 - 14:40 | Lab 2: Implement a feature (70 min)

**Learning theme:** disciplined AI-assisted development using the tools you built this morning

### Core -- Guided

1. Open your feature issue from the pre-lab exercise
2. Open the API file for your language in the Codespace
3. At the top of the file, write a comment describing the endpoint you are adding (method, path, input, output) -- this gives Copilot inline context
4. Use inline completion to write the endpoint function body, accepting suggestions selectively
5. If a suggestion is wrong, reject it (Escape) and add more context in a comment before trying again
6. Open the shared frontend file that calls the API
7. Use inline completion to connect the frontend to your new endpoint
8. Run the app and verify the feature works in the browser
9. Commit with a message that references your issue number (e.g., `feat: add X endpoint, closes #1`)

Expected output: feature implemented, working in the browser, committed and referencing the issue.

### Core -- Self-directed

Implement the feature described in your issue using Copilot inline completions and your instructions file. No chat. The feature should be visible in the running app and committed to the repo.

### Extra Challenge -- Guided

1. Open Copilot Chat (one premium request)
2. Attach your feature prompt file explicitly as context
3. Ask Copilot to review your implementation and suggest input validation
4. Add the suggested validation, reviewing each suggestion before accepting
5. Ask Copilot Chat to write one test for your endpoint (second premium request)
6. Run the test and fix any failures
7. If the async coding agent PR from the demo has arrived, open it and compare its implementation to yours

### Extra Challenge -- Self-directed

Add validation, error handling, and at least one test to your implementation, using whatever combination of inline completion and chat you think is most efficient. If the coding agent PR has arrived, review it critically: where did it do better than you, and where did it miss something?

---

## 14:40 - 15:00 | PRs, review, and AI-generated code (20 min)
...
## 15:15 - 15:35 | Vibecoding and GitHub Spark (20 min)

**Mode:** instructor demo only
...
**Discussion (3 min):** when is this appropriate? Good answers: prototypes, personal tools, throwaway scripts, early-stage exploration. Bad answers: anything with auth, user data, or production traffic.

**Backup:** have a recorded screen capture ready. A 20-minute slot with no fallback is a 20-minute hole.

---

## 15:35 - 16:50 | Lab 3: CI and Deploy with GitHub Actions (75 min)

**Learning theme:** the full pipeline -- automation as protection, deployment as the result

### Explain (15 min)

Easy: what CI does and why it matters more when AI is generating diffs at high volume. How GitHub Pages deployment fits into a GitHub Actions workflow as a natural extension of CI.
...
1. In your repo, go to Settings > Pages, set the source to "GitHub Actions"
2. Open the provided CI workflow file at `.github/workflows/ci.yml`
3. Read through each job and step before touching anything -- add a comment to any line you do not understand
4. Adjust the language filter in the matrix to match your chosen language folder
5. Commit the workflow file to a new branch and open a PR
6. Watch the Actions tab -- observe the workflow run and identify which steps pass and fail
7. Fix any failures (the most common are path issues in the language filter)
8. Once CI is green, open the provided deploy workflow at `.github/workflows/deploy.yml`
9. Read through it the same way -- comment anything unclear
10. Merge your PR to main
11. Watch the deploy workflow run in the Actions tab
12. Open the Pages URL (Settings > Pages shows it) and verify your feature is live

Expected output: a live URL with your feature deployed via GitHub Actions.

### Core -- Self-directed

Get your feature deployed to GitHub Pages via GitHub Actions. CI must pass before deployment runs. Both workflows are provided in the repo -- understand what each step does before enabling it.

### Extra Challenge -- Guided

1. In the CI workflow, add a matrix strategy to test across multiple versions of your language runtime
2. Add a lint or formatting step using the appropriate tool for your language (ESLint for Node, Ruff for Python, dotnet format for .NET)
3. Add a step that posts a comment on the PR with the Pages preview URL when a branch is deployed
4. In a new branch, intentionally introduce a bug and open a PR -- verify that CI catches it before it can be merged

### Extra Challenge -- Self-directed

Extend the provided workflows to add at least two of the following: multi-version matrix testing, a lint step, a PR preview deployment, or a post-deployment smoke test that pings the live URL and fails the workflow if the response is not 200. Break something intentionally and verify CI catches it.
...
|---|---|---|---|
| Welcome, check, decision | 09:00 | 09:20 | 20 min |
| Lab 0: Codespaces + debrief | 09:20 | 10:15 | 55 min |
| Break | 10:15 | 10:30 | 15 min |
| Copilot tiers, limits, demo | 10:30 | 11:00 | 30 min |
| Lab 1: Instructions, prompts, skills demo | 11:00 | 12:00 | 60 min |
| Morning wrap | 12:00 | 12:10 | 10 min |
| Lunch | 12:10 | 13:10 | 60 min |
| Issues + agentic demo | 13:10 | 13:30 | 20 min |
| Lab 2: Implement a feature | 13:30 | 14:40 | 70 min |
| PRs + review + discussion | 14:40 | 15:00 | 20 min |
| Break | 15:00 | 15:15 | 15 min |
| Vibecoding + Spark demo | 15:15 | 15:35 | 20 min |
| Lab 3: CI + Deploy | 15:35 | 16:50 | 75 min |
| Wrap-up | 16:50 | 17:00 | 10 min |
| **Net teaching time** | | | **6h 30min** |
| **Breaks + lunch** | | | **1h 30min** |
| **Total** | | | **8h** |
...
**The two-mode format adds prep work.** Every lab now needs two written versions. For Lab 3 alone that is four documents (Core guided, Core self-directed, Extra Challenge guided, Extra Challenge self-directed). Budget time to write these before the workshop, and put them in the repo so participants can access them from their Codespace without switching windows.

**Lab 3 is the tightest lab.** 75 minutes for CI setup, debugging a failing workflow, plus deployment is realistic but not generous. The most common failure mode is participants spending 20 minutes on a path error in the language matrix and not getting to deployment. Consider whether the deploy workflow should be provided pre-configured and participants just enable it, so CI gets the full attention and deployment is a reward rather than another debugging session.

**The Spark demo placement is better here.** Putting it before Lab 3 rather than after means it functions as a palate cleanser and reframe before the most technical lab, rather than a rushed afterthought following a deployment that may or may not have worked. If the Spark demo runs long, you lose prep time for Lab 3 -- enforce the 20-minute limit.